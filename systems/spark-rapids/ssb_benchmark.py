#!/usr/bin/env python3
"""
SSB Benchmark Driver for Spark-RAPIDS
Usage:
    spark-submit ssb_benchmark.py --mode gpu
    spark-submit ssb_benchmark.py --mode cpu
"""
import argparse
import math
import os
...
SSB_PARQUET_BASE = os.environ.get(
    "SSB_PARQUET_BASE",
    f"/export/data1/ssb/duckdb/sideways/parquet/sf{SF}",
)
QUERY_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ssb_queries")
...
TABLES = ["lineorder", "customer", "date", "part", "supplier"]
QUERIES = [
    "q11", "q12", "q13",
    "q21", "q22", "q23",
    "q31", "q32", "q33", "q34",
...
def create_spark_session(mode):
    builder = SparkSession.builder.appName(f"SSB Benchmark ({mode.upper()})")
    if mode == "cpu":
        builder = (
...
def verify_query(qname, result_file, answer_file):
    ...
    def read_lines(path):
        ...
        return [l.replace('"', '') for l in lines[1:]]
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="SSB Spark-RAPIDS Benchmark")
    ...
    log("=" * 60)
    log(f"  {ver_label} (mode={args.mode.upper()}) -- SSB SF{SF}")
    log("=" * 60)
    log(f"Data: {SSB_PARQUET_BASE}")
    ...
    # ---- Register Tables ----
    log("Registering SSB tables ...")
    register_tables(spark, SSB_PARQUET_BASE)
    log("")
    ...
    answer_dir = f"{ANSWERS_BASE}/ssb/sf{SF}"
    verify_fail = False
    for qname in args.queries:
    ...
    lines = []
    lines.append(
        f"=== {ver_label} (mode={args.mode.upper()}) -- SSB SF{SF} ==="
    )
