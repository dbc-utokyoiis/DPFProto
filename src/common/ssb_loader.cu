...
enum class ExecutionMode {
    ...
    UNCOMP,              // Uncompressed baseline (all modes can read)
};
...
std::array<CompressionMethod, SSB::common::SSB_MAX_NFIELDS> golap_compression_methods = {};
...
struct SamplingTask {
    ...
    std::span<const size_t, NFields> csv_column_map; /* maps storage field → CSV column */
    bool enable_lbc;
...
struct ColumnLoadTask {
    ...
    std::span<const size_t, NFields> csv_column_map; /* maps storage field → CSV column */
    std::span<const size_t, NFields> field_enum_indices; /* maps sequential active field index → enum value */
    //std::span<const size_t, NFields> field_encoded_sizes;
    //std::span<const enum rec_type, NFields> field_encoded_types;
...
// TODO: CHAR(1) only. For CHAR(2), need sv[0] | (sv[1] << 8).
...
/**
 * ssb_filter_csv_fields: Extract active fields from a full CSV row.
 * csv_column_map[i] gives the CSV column index for storage field i.
 * If csv_column_map is empty, returns all_fields unchanged.
 */
template <size_t NFields>
inline std::vector<std::string_view> ssb_filter_csv_fields(
    const std::vector<std::string_view>& all_fields,
    std::span<const size_t, NFields> csv_column_map) {
    if constexpr (NFields == 0) return all_fields;
    if (csv_column_map.empty()) return all_fields;
    std::vector<std::string_view> result;
    result.reserve(NFields);
    for (size_t i = 0; i < NFields; i++) {
        result.push_back(all_fields[csv_column_map[i]]);
    }
    return result;
}

void metadata_set_free_page_id(SSBTableMetadata &metadata, size_t next_page_id)
{
    metadata.free_page_id = next_page_id;
}

void metadata_set_nrows(SSBTableMetadata &metadata, SSB::common::Table table, uint64_t num_rows)
{
    switch (table)
    {
    ...
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_nrows = num_rows;
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_nrows = num_rows;
        break;
...
void metadata_set_prefix_sum_chunk_size(SSBTableMetadata &metadata, SSB::common::Table table, uint64_t npages)
{
    switch (table)
    {
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_prefix_sum_chunk_size = npages;
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_prefix_sum_chunk_size = npages;
        break;
    case SSB::common::Table::PART:
...
size_t metadata_set_prefix_sum_nrecs_per_page(SSBTableMetadata &metadata, SSB::common::Table table, 
    std::vector<int> &output_fds,
    size_t page_size,
    size_t i,
    ...
    std::vector<uint64_t>& prefix_sum_nrecs_per_page)
{
    ...
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_prefix_sum_start_page_ids[i] = start_page_id;
        metadata.table_lineorder_prefix_sum_npages[i] = npages_for_prefix_sum;
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_prefix_sum_start_page_ids[i] = start_page_id;
        metadata.table_date_prefix_sum_npages[i] = npages_for_prefix_sum;
        // metadata.table_date_nrecs_per_page_prefix_sum[i] = nrecs_per_page;
        break;
    case SSB::common::Table::PART:
...
void metadata_set_page_id(SSBTableMetadata &metadata, SSB::common::Table table, int i, uint64_t start_page_id, uint64_t npages)
{
    switch (table)
    {
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_start_page_ids[i] = start_page_id;
        metadata.table_lineorder_npages[i] = npages;
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_start_page_ids[i] = start_page_id;
        metadata.table_date_npages[i] = npages;
        break;
    case SSB::common::Table::PART:
...
void metadata_set_max_nrows_in_page(SSBTableMetadata &metadata, SSB::common::Table table, size_t i, uint32_t nrows)
{
    switch (table)
    {
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_max_nrows_in_page[i] = nrows;
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_max_nrows_in_page[i] = nrows;
        break;
    case SSB::common::Table::PART:
...
void metadata_set_prefix_sum_start_page_ids(SSBTableMetadata &metadata, SSB::common::Table table, size_t i, uint64_t start_page_id)
{
    switch (table)
    {
    case SSB::common::Table::CUSTOMER:
...
void metadata_set_prefix_sum_npages(SSBTableMetadata &metadata, SSB::common::Table table, size_t i, uint64_t start_page_id)
{
    switch (table)
    {
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_prefix_sum_npages[i] = start_page_id;
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_prefix_sum_npages[i] = start_page_id;
        break;
    case SSB::common::Table::PART:
...
size_t metadata_set_no_compression(SSBTableMetadata &metadata, SSB::common::Table table, size_t i) {
    switch (table)
    {
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_compression_method[i] = static_cast<uint16_t>(CompressionMethod::NONE);
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_compression_method[i] = static_cast<uint16_t>(CompressionMethod::NONE);
        break;
    case SSB::common::Table::PART:
...
size_t metadata_set_compressed_page_sizes(SSBTableMetadata &metadata, SSB::common::Table table,
    std::vector<int> &output_fds,
    size_t page_size,
    ...
    std::vector<uint32_t>& compressed_page_sizes)
{
    ...
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_compressed_page_sizes_start_page_ids[i] = start_page_id;
        metadata.table_lineorder_compressed_page_sizes_npages[i] = npages_for_compressed_pages;
        metadata.table_lineorder_compression_method[i] = static_cast<uint16_t>(compression);
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_compressed_page_sizes_start_page_ids[i] = start_page_id;
        metadata.table_date_compressed_page_sizes_npages[i] = npages_for_compressed_pages;
        metadata.table_date_compression_method[i] = static_cast<uint16_t>(compression);
        break;
    case SSB::common::Table::PART:
...
template<typename T>
size_t metadata_set_stats(SSBTableMetadata &metadata, SSB::common::Table table, 
    std::vector<int> &output_fds,
    size_t page_size,
    size_t i,
    const uint64_t start_page_id,
    std::vector<Stats<T>>& stats)
{
    ...
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        // metadata.table_lineorder_enable_stats[i] = 1;
        metadata.table_lineorder_nstats[i] = stats.size();
        metadata.table_lineorder_stats_npages[i] = npages_for_stats;
        metadata.table_lineorder_stats_start_page_ids[i] = start_page_id;
        break;
    case SSB::common::Table::DDATE:
        // metadata.table_date_enable_stats[i] = 1;
        metadata.table_date_nstats[i] = stats.size();
        metadata.table_date_stats_npages[i] = npages_for_stats;
        metadata.table_date_stats_start_page_ids[i] = start_page_id;
        break;
    case SSB::common::Table::PART:
    ...
    if (npages_for_stats == 0) {
        std::cerr << "[Unexpected] npages_for_stats is 0!" << std::endl;
        exit(EXIT_FAILURE);
    }
...
template<typename T>
size_t metadata_set_sideways_stats(SSBTableMetadata &metadata, SSB::common::Table table,
    std::vector<int> &output_fds,
    size_t page_size,
    size_t i, size_t j,
    const uint64_t start_page_id,
    std::vector<Stats<T>>& stats)
{
    ...
    {
    case SSB::common::Table::LINEORDER:
        if (i > SSB::common::kLineOrderFieldCount) {
            std::cerr << "[FATAL] i exceeds kLineOrderFieldCount!" << std::endl;
            exit(EXIT_FAILURE);
        }
        if (j > SSB::common::kLineorderSidewaysCount) {
            std::cerr << "[FATAL] j exceeds kLineorderSidewaysCount!" << std::endl;
            exit(EXIT_FAILURE);
        }
        metadata.table_lineorder_sideways_nstats[i][j] = stats.size();
        metadata.table_lineorder_sideways_stats_npages[i][j] = npages_for_stats;
        metadata.table_lineorder_sideways_stats_start_page_ids[i][j] = start_page_id;
        break;
    default:
        std::cerr << "[FATAL] Sideways stats are only supported for LINEORDER table." << std::endl;
        exit(EXIT_FAILURE);
        break;
    }
    if (npages_for_stats == 0) {
...
void metadata_set_no_stats(SSBTableMetadata &metadata, SSB::common::Table table, size_t i)
{
    switch (table)
    {
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_nstats[i] = 0;
        metadata.table_lineorder_stats_npages[i] = 0;
        metadata.table_lineorder_stats_start_page_ids[i] = 0;
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_nstats[i] = 0;
        metadata.table_date_stats_npages[i] = 0;
        metadata.table_date_stats_start_page_ids[i] = 0;
        break;
    case SSB::common::Table::PART:
...
size_t metadata_set_compression_base_page_id(SSBTableMetadata &metadata, SSB::common::Table table,
    std::vector<int> &output_fds,
    size_t page_size,
    int i,
    ...
    std::vector<uint64_t> &base_start_page_ids)
{
    ...
    case SSB::common::Table::CUSTOMER:
        ...
        break;
    case SSB::common::Table::LINEORDER:
        metadata.table_lineorder_compression_base_start_page_ids[i] = start_page_id;
        metadata.table_lineorder_compression_nbases[i] = nbases;
        break;
    case SSB::common::Table::DDATE:
        metadata.table_date_compression_base_start_page_ids[i] = start_page_id;
        metadata.table_date_compression_nbases[i] = nbases;
        break;
    case SSB::common::Table::PART:
    ...
    size_t npages = SSB::nbase_to_npages(nbases, page_size);
    ...
    const size_t field_id, const int line_from)
{
    ...
    if (compression == CompressionMethod::NONE)  {
        ...
        if (ctx.enable_golap_compression_mode) {
            ...
            switch (compression) {
                /* PFOR/PFOR64/FSST_ROWID compression is done in flush_staging_buffer */
        ...
        if (compressed_size == 0 || compressed_size > page_size) {
            std::cerr << "[ERROR](" << line_from << ") Invalid compressed size: " << compressed_size
                      << " (page_id=" << page_id << ", field_id=" << field_id << ", page_size=" << page_size << ")\n";
            exit(1);
        }
        size_write = roundup4096(compressed_size);
        target_buffer = ctx.dst_buffer;
        // std::cout << "[DEBUG] " << page_buffer[0] << page_buffer[1] << page_buffer[2] << page_buffer[3] << std::endl;
        // std::cout << "[DEBUG] " << ctx.dst_buffer[0] << ctx.dst_buffer[1] << ctx.dst_buffer[2] << ctx.dst_buffer[3] << std::endl;
        // exit(0);
        // V2
    ...
    if (compression == CompressionMethod::PFOR ||
        ...
        // For PFOR/PFOR64/FSST_ROWID, nalloc is set during compression
...
void flush_staging_buffer(
    ...
    while (processed < buffer.size())
    {
        ...
        if (batch_size > 0)
        {
            ...
            else if constexpr (std::is_same_v<T, CharAsInt>)
            {
                ...
                if (compression != CompressionMethod::NONE) {
                    ...
                    ret = compress_int_with_pfor(
                        ...
                        comp_ctx.workspaces, comp_ctx.compressed_size);
                    if (ret < 0) {
                        std::cerr << "[ERROR](" << __LINE__ << ") PFOR compression failed (CharAsInt)\n";
                        exit(EXIT_FAILURE);
                        return;
                    }
                    ...
                    comp_ctx.compressed_size = compressed_page_size;
                    if (ret == PAG_SLOTID_MASK_ERROR) {
                        std::cerr << "[ERROR](" << __LINE__ << ") PFOR pagcol append failed (CharAsInt)\n";
                        exit(EXIT_FAILURE);
                    }
                } else {
                    pagcol_append_batch_unordered_column_int32(page_buffer, vals, page_size);
            ...
            // FSST_ROWID batch compression for CharType/VCharType
            else if constexpr ((std::is_same_v<T, CharType> || std::is_same_v<T, VCharType>))
            {
                if (compression == CompressionMethod::FSST_ROWID) {
                    ...
                    // Classic Loop for Char, VarChar (non-FSST)
                    for (size_t k = 0; k < batch_size; ++k)
                    {
...
void process_column_logic(
    ...
    if (record_buffer_vec) {
        ...
        if constexpr (!std::is_same_v<T, VCharType>) {
            ...
            size_t rec_base_size = val.value.size();

            if (rec_base_size > freespace) {
              buffered_page_count++;
    ...
    for (size_t i = 0; i < NFields; i++) {
        ...
        switch (attr_type) {
            ...
            case rec_type::REC_ATTR_CHAR:
            {
                ...
                //std::cout << "Field " << i << ": rec_size=" << rec_size
                //  << ", rec_base_size=" << rec_base_size
                //  << ", nrecs_per_page=" << nrecs_per_page << "\n";
                //exit(1);
                break;
            }
        ...
        if (task.verbose)
            std::cout << "Field " << i << ": freespace=" << freespaces_init[i] << "\n";
    }
    for (size_t vf = 0; vf < NumVarCharFields; vf++) {
    ...
    if (task.golap_compression_mode) {
        ...
        for (size_t sp = 0; sp < NUM_SAMPLE_POSITIONS; sp++) {
            ...
            while (std::getline(file, line)) {
                ...
                auto fields = ssb_filter_csv_fields<NFields>(tpch_split_row(line), task.csv_column_map);
                size_t f = 0;
            ...
            for (size_t j = 0; j < NCompressionMethods; j++) {
                ...
                if (task.verbose)
                    std::cout << "[Sampling][File " << task.id << "][worst] Field " << i
                              ...
                              << ", Worst Compressed Size: " << compressed_page_sizes[i][j]
                              << ", Worst Ratio: " << worst_ratio << std::endl;
            }
        }
            ...
            for (const auto& part : fields) {
                ...
                switch (attr_type) {
                    ...
                    case rec_type::REC_ATTR_VCHAR:
                    {
                        ...
                        // size_t len_total = sizeof(uint32_t) + 2 * sizeof(uint16_t) + len_vchar_aligned + sizeof(uint64_t);I
...
static std::vector<ColumnLoadTask<NFields, NumStartXtns, NumVarCharFields, NumFilters, NumSidewaysFilters>> generate_column_tasks(
    ...
    const std::array<size_t, NFields>& field_enum_indices,
    size_t *npages_expected)
{
    std::cout << "NumStartXtns: " << NumStartXtns << ", NFields: " << NFields << ", NumVarCharFields: " << NumVarCharFields << std::endl;
    ...
    /* UNCOMP mode: override all compression methods to NONE */
    std::array<CompressionMethod, NFields> uncomp_methods{};
    uncomp_methods.fill(CompressionMethod::NONE);
    const auto& effective_compression = (options.execution_mode == ExecutionMode::UNCOMP)
        ? uncomp_methods : compression_methods;
...

    {
        ...
        for (auto& path : paths) {
            struct SamplingTask<NFields, NumVarCharFields> task{
                ...
                .csv_column_map = std::span(csv_column_map),
    ...
    if (options.verbose) std::cout << "Total linecount is " << count_sum << std::endl;
    for (size_t i = 0; i < ntasks; ++i) {
        for (size_t j = 0; j < NFields; ++j) {
            if (options.verbose)
                std::cout << "File " << paths[i] << ", Field " << j
    ...
    for (size_t i = 0; i < NumVarCharFields; ++i) {
        for (const auto& len : varchar_cluster_thresholds[i]) {
            std::cout << "LBC Thresholds for VarChar Field " << i << ": " << len << "\n";
        }
    }
...
#if 1
    {
        ...
        for (size_t i = 0; i < NFields; i++) {
            ...
            metadata_set_page_id(metadata, table, field_enum_indices[i], start_page_id + npages_col_sum, npages_estimated);
    ...
    for (size_t i = 0; i < NFields; i++) {
        ...
        /* calcurate npages based on the result of sampling */
        size_t npages_estimated = npages_sum;
        std::cout << "Expected number of pages for field " << i << " (enum=" << field_enum_indices[i] << "): " << npages_estimated << std::endl;
#if 0
        /* Debug print */
        for (size_t j = 0; j < ntasks; ++j) {
            std::cout << "\tTask[" << j << "], start_page_id: " << start_page_ids[j][i] << std::endl;
...
#endif
        std::cout << "\tfield size: " << field_sizes[i] << "\tstart_page_id: " << start_page_id + npages_col_sum
                << "\tnpages_sum: " << npages_sum << std::endl;
    ...
    for (size_t i = 0; i < paths.size(); ++i) {
        ...
        ColumnLoadTask<NFields, NumStartXtns, NumVarCharFields, NumFilters, NumSidewaysFilters> task{
            .field_sizes = std::span(field_sizes),
            .field_types = std::span(field_types),
            .compression_methods = std::span(effective_compression),
            .csv_column_map = std::span(csv_column_map),
            .field_enum_indices = std::span(field_enum_indices),
            .varchar_field_indexes = std::span(varchar_field_indexes),
        ...
        task.enable_fsst = options.enable_fsst;
    ...
    std::cout << "Total number of pages: " << npages_col_sum << std::endl;
    ...
    *npages_expected = npages_col_sum + npages_dict_sum;
    ...
    for (size_t i = 0; i < load_tasks.size(); ++i) {
        ...
        //for (size_t j = 0; j < NumVarCharFields + 1; ++j) {
        //    if (j == 0 || dict_encoding) {
        //        std::cout << ", start_page_id[" << j << "] = " << load_tasks[i].start_page_ids[j];
        //    }
        //}
...
static SSBLoaderThreadStats<MaxNumBuffers, NFields> load_lines_to_table_as_column_with_sideways_stats_golap(
    ...
    const int dryrun, const int verbose)
{
    ...
    SSBTableMetadata &metadata = *task.metadata;
    ...
    /* Dict encoding maps for sideways CHAR fields (string → int32 ID) */
    std::array<std::map<std::string, int32_t>, SSB::common::kSidewaysDictMapCount> sideways_dict_maps {};
    if constexpr (NumSidewaysFields > 0) {
        if constexpr (std::is_same_v<EnumType, SSB::common::LineOrderField>) {
            SSB::common::ssb_build_sideways_dict_encoding_maps(sideways_dict_maps);
        }
    }
    ...
    while (std::getline(file, line)) {
        ...
        auto all_csv_fields = tpch_split_row(line);
        auto fields = ssb_filter_csv_fields<NFields>(all_csv_fields, task.csv_column_map);
        /* Append sideways columns in LSS enum order (= CSV column order) */
        if constexpr (NumSidewaysFields > 0) {
            for (size_t sw = 0; sw < NumSidewaysFields; sw++) {
                size_t csv_idx = SSB::common::kLineOrderFieldCount + sw;
                if (csv_idx < all_csv_fields.size()) {
                    fields.push_back(all_csv_fields[csv_idx]);
                }
            }
        }
            ...
            if (fi < NFields) {
                ...
                case rec_type::REC_ATTR_CHAR: {
                    ...
                    for (size_t j = 0; j < NFields; j++) {
                        ...
                        auto it = sideways_dict_maps[scf].find(part_str);
                        if (it != sideways_dict_maps[scf].end()) {
                            update_stats<int32_t>(sw_stats[f_sw], it->second);
                        } else {
                            std::cerr << "[ERROR] Dictionary encoding map lookup failed for sideways char field index: " << scf
                                      << " value: \"" << part << "\"\n";
                            exit(EXIT_FAILURE);
                        }
                    }
...
template <typename EnumType, size_t MaxNumBuffers, size_t NumStartXtns, size_t NFields,
    size_t NumVarCharFields, size_t NumFilters, size_t NumSidewaysFields>
static SSBLoaderThreadStats<MaxNumBuffers, NFields> load_lines_to_table_as_column_with_sideways_stats(
    ColumnLoadTask<NFields, NumStartXtns, NumVarCharFields, NumFilters, NumSidewaysFields>& task,
    std::vector<std::string>& paths,
    std::vector<int>& output_fds,
    const std::array<char*, MaxNumBuffers>& buffers,
    ...
    const int dryrun, const int verbose)
{
    ...
    all_stats.reserve(NFields);
    {
        for (size_t f = 0; f < NFields; f++) {
            ...
            switch (t) {
                ...
                case rec_type::REC_ATTR_CHAR:
                    if (field_sizes[f] < 3) {
                        /* CHAR(1-2) always stored as int32 (CharAsInt) for all modes */
    ...
    for (size_t i = 0; i < NFields; ++i) {
        if (compress) {
            if (enable_golap_compression_mode) {
                ...
                /* CharAsInt: CHAR(1-2) stored as INT32 + PFOR
                 * Only when base compression is not NONE (e.g. DATE table stays uncompressed) */
                if (field_types[i] == rec_type::REC_ATTR_CHAR && field_sizes[i] < 3
                    && enable_fsst && compression[i] != CompressionMethod::NONE) {
                    compression[i] = CompressionMethod::PFOR;
                }
            }
            if (verbose) std::cout << "Field " << i << " compression method: "
                      << compression_method_name(compression[i]) << std::endl;
    ...
    /* For FSST_ROWID: CHAR fields need staging buffer to accumulate batch */
    bool has_fsst_fields = false;
    for (size_t i = 0; i < NFields; ++i) {
        if (compression[i] == CompressionMethod::FSST_ROWID) {
            has_fsst_fields = true;
            break;
        }
    }
    ...
    for (size_t i = 0; i < NFields; ++i) {
        ...
        if ((compress || enable_stats[i])
            ...
            switch (attrtype) {
                ...
                case rec_type::REC_ATTR_CHAR:
                    if (field_sizes[i] < 3) {
                        /* CharAsInt: same as INT32, 128-aligned */
                        nrecs_per_sort_buffer = freespace / sizeof(int32_t);
                        if (nrecs_per_sort_buffer >= 128) {
                            nrecs_per_sort_buffer = (nrecs_per_sort_buffer / 128) * 128;
                        } else {
                            nrecs_per_sort_buffer = 128;
                        }
                    } else if (compression[i] == CompressionMethod::FSST_ROWID) {
                        /* FSST CHAR(>=3): use aligned field size (no rowid overhead) */
                        constexpr size_t fsst_alignment = 4;
                        size_t rec_size_est = (field_sizes[i] % fsst_alignment == 0)
                            ? field_sizes[i] : field_sizes[i] + (fsst_alignment - (field_sizes[i] % fsst_alignment));
    ...
    /* Uncomp mode: skip staging entirely — all fields use direct pagcol_append path */
    if (compress)
    for (size_t i = 0; i < NFields; ++i) {
        // should_buffer ->
        ...
        if ((compression[i] != CompressionMethod::NONE) || enable_stats[i]) {
            switch (field_types[i]) {
                ...
                case rec_type::REC_ATTR_CHAR: staging_line_buffers[i] = std::vector<SortPair<CharType>>{}; break;
...
#else
        auto attrtype = field_types[i];
        /* FIXME add proposed varchar case */
        bool need_staging = (attrtype == REC_ATTR_INT16 ||
            attrtype == REC_ATTR_INT32 ||
            attrtype == REC_ATTR_INT64 ||
            attrtype == REC_ATTR_DECIMAL ||
            attrtype == REC_ATTR_DATE)
            && (enable_stats[i] || compression[i] != CompressionMethod::NONE);
        /* FSST_ROWID CHAR/VCHAR fields also need staging for batch compression */
        need_staging = need_staging
            || ((compression[i] == CompressionMethod::FSST_ROWID)
                && (attrtype == REC_ATTR_CHAR || attrtype == REC_ATTR_VCHAR));
        /* CharAsInt: CHAR(1-2) with PFOR also needs staging */
        need_staging = need_staging
            || ((compression[i] == CompressionMethod::PFOR)
        ...
        for (const auto& part : fields) {
            ...
            if (fi < NFields) {
                ...
                case rec_type::REC_ATTR_CHAR: {
                    ...
                    auto it = sideways_dict_maps[scf].find(part_str);
                    if (it == sideways_dict_maps[scf].end()) {
                        std::cerr << "[ERROR] Sideways dict lookup failed for field " << scf
                                  << ": \"" << part << "\"\n";
                        exit(EXIT_FAILURE);
                    }
                    int32_t dict_id = it->second;
                    for (size_t j = 0; j < NFields; j++) {
                        if (field_types[j] == rec_type::REC_ATTR_VCHAR && enable_lbc) {
                            for (size_t vf = 0; vf < NumVarCharFields; vf++) {
                                for (size_t cid = 0; cid < varchar_cluster_thresholds[vf].size(); cid++) {
                                    auto& sw_stats = std::get<std::vector<ColumnStats<int32_t>>>(sideways_stats_varchar[vf][cid]);
                                    update_stats<int32_t>(sw_stats[f_sw], dict_id);
                                }
                            }
                        } else {
                            auto& sw_stats = std::get<std::vector<ColumnStats<int32_t>>>(sideways_stats[j]);
    ...
    for (size_t f = 0; f < NFields; ++f) {
        ...
        if (staging_line_buffers[f].has_value()) {
            switch (field_types[f]) {
                ...
                case rec_type::REC_ATTR_VCHAR: {
                    /* NOTE: buffering is basically disabed on VCHAR */
                    std::cerr << "[ERROR](" << __FILE__ << ":" << __LINE__ << ") Unexpected buffering on VCHAR field at final flush. Stop.\n";
                    exit(EXIT_FAILURE);
...
static SSBLoaderThreadStats<MaxNumBuffers, NFields> load_lines_to_table_as_column(
    ColumnLoadTask<NFields, NumStartXtns, NumVarCharFields, NumFilters, NumSidewaysFilters>& task,
    std::vector<std::string>& paths,
    std::vector<int>& output_fds,
    const std::array<char*, MaxNumBuffers>& buffers,
    const size_t page_size, const int dryrun, const int verbose)
{
    ...
    if (enable_sideways_stats) {
        if (enable_golap_compression_mode) {
            if constexpr (std::is_same_v<EnumType, SSB::common::LineOrderField>) {
                return load_lines_to_table_as_column_with_sideways_stats_golap<EnumType, MaxNumBuffers, NumStartXtns, NFields,
                    NumVarCharFields, NumFilters, SSB::common::kLineorderSidewaysCount>(
                        task, paths, output_fds, buffers, page_size,
                        SSB::common::fmt.lineorder_sideways_information_types,
                        SSB::common::fmt.lineorder_sideways_information_sizes,
                        dryrun, verbose);
            }
        } else {
            if constexpr (std::is_same_v<EnumType, SSB::common::LineOrderField>) {
        ...
        /* Other tables will be loaded as normal (no sideways stats) */
    }
    /*
     else {
    ...
    {
        for (size_t i = 0; i < NFields; i++) {
            sideways_stats[i] = std::vector<ColumnStats<int32_t>>(NumSidewaysFilters);
...
#else
        auto attrtype = field_types[i];
        bool need_staging_lo = ((attrtype == REC_ATTR_INT16 ||
            attrtype == REC_ATTR_INT32 ||
            attrtype == REC_ATTR_INT64 ||
            attrtype == REC_ATTR_DECIMAL ||
            attrtype == REC_ATTR_DATE)
    ...
    #if 0
    auto dict_types = SSB::common::fmt.dict_types;
    #endif
    ...
    while (std::getline(file, line)) {
        ...
        /* Parse sideways columns from the denormalized CSV and update sideways stats.
         * Sideways columns start at CSV index kLineOrderFieldCount (=17) in the
         * full CSV row from 02_ssb_sideways_pruning.sh. */
        if constexpr (NumSidewaysFilters > 0) {
            if (enable_sideways_stats &&
                all_csv_fields.size() >= SSB::common::kLineOrderFieldCount + NumSidewaysFilters) {
                for (size_t sw = 0; sw < NumSidewaysFilters; sw++) {
                    std::string_view part = all_csv_fields[SSB::common::kLineOrderFieldCount + sw];
                    std::string part_str(part);
                    auto it = sideways_dict_maps[sw].find(part_str);
                    if (it == sideways_dict_maps[sw].end()) {
                        std::cerr << "[ERROR] Sideways dict lookup failed for field " << sw
                                  << ": \"" << part << "\"\n";
                        exit(EXIT_FAILURE);
                    }
                    int32_t dict_id = it->second;
                    for (size_t j = 0; j < NFields; j++) {
                        auto& sw_stats = std::get<std::vector<ColumnStats<int32_t>>>(sideways_stats[j]);
                        update_stats(sw_stats[sw], dict_id);
                    }
                }
            }
        }
...
uint64_t load_multiple_files_to_table_column(
    LoaderOptions &options, SSBTableMetadata &metadata, SSB::common::Table table,
    ...
    StatEntry &entry)
{
    ...
    std::vector<std::future<SSBLoaderThreadStats<MaxNumBuffers, NumRecBufs>>> futures;
    std::vector<std::thread> threads;
    ...
    for (size_t i = 0; i < num_tasks; ++i) {
        auto load_func = static_cast<SSBLoaderThreadStats<MaxNumBuffers, NumRecBufs>(*)(
        ...
        std::packaged_task<SSBLoaderThreadStats<MaxNumBuffers, NumRecBufs>()> task(
            std::bind(load_func,
    ...
    std::array<std::vector<uint64_t>, NumVarCharFields> prefix_sum_nrecs_per_page_varchar = {};
    {
        ...
        for (size_t i = 0 ; i < NFields; ++i) {
            ...
            npages_sum += metadata_set_prefix_sum_nrecs_per_page(
                ...
                tasks[0].field_enum_indices[i], start_base_page_id + npages_sum,
                prefix_sum_nrecs_per_page[i]
            );
        }
        std::cout << "Total pages used for prefix sum nrecs: from " << *npages_used_sum;
        *npages_used_sum += npages_sum;
        std::cout << " to " << *npages_used_sum << std::endl;
    ...
    if constexpr (NumFilters > 0) {
        ...
        for (size_t i = 0 ; i < NFields; ++i) {
            ...
            if (enable_stats) {
                ...
                metadata_set_no_stats(metadata, table, tasks[0].field_enum_indices[i]);
            }
        }
...
#endif
                        if constexpr (std::is_same_v<EnumType, SSB::common::LineOrderField>) {
                            if (field_index == SSB::common::LineOrderField::LO_ORDERDATE) {
            ...
            for (size_t i = 0 ; i < NFields; ++i) {
                ...
                for (size_t j = 0 ; j < NumSidewaysFilters; ++j) {
                    if (varchar_stats) {
                        ...
                        if constexpr (std::is_same_v<EnumType, SSB::common::LineOrderField>) {
                            stats_field_type = SSB::common::fmt.lineorder_sideways_information_types[j];
                        } else {
                        ...
                        switch (stats_field_type) {
                            ...
                            npages_sum += metadata_set_sideways_stats(
                                metadata, table,
                                options.output_fds, options.page_size,
                                tasks[0].field_enum_indices[i], j,
                                start_base_page_id + npages_sum,
                                converted_hist
                            );
                            for (const auto& s : converted_hist) {
                                if (options.verbose) std::cout << "Field " << i << " (enum=" << tasks[0].field_enum_indices[i] << "), Sideways Filter " << j
                    ...
                    std::cout << "Total pages used for sideways information - "
    ...
    std::array<CompressionMethod, NFields> actual_compression_methods {};
    ...
    if (options.compress) {
        /* persist compressed page sizes */
        {
            ...
            for (size_t i = 0; i < NFields; ++i)
            {
                ...
                else
                {
                    ...
                    /* CharAsInt: CHAR(1-2) stored as INT32 + PFOR
                     * Only when base compression is not NONE */
                    if (tasks[0].field_types[i] == rec_type::REC_ATTR_CHAR && tasks[0].field_sizes[i] < 3
                        && options.enable_fsst && compression != CompressionMethod::NONE) {
                        compression = CompressionMethod::PFOR;
                    }
                }
                actual_compression_methods[i] = compression;

                if (compression == CompressionMethod::NONE)
                {
                    std::cout << "Field " << i << " (enum=" << tasks[0].field_enum_indices[i] << ") has no compression. Skipping persisting compressed page sizes." << std::endl;
                    metadata_set_no_compression(metadata, table, tasks[0].field_enum_indices[i]);
                }
                else
                {
                    enum rec_type field_type = tasks[0].field_types[i];
                    ...
                    npages_sum += metadata_set_compressed_page_sizes(
                        metadata, table,
                        options.output_fds, options.page_size,
                        tasks[0].field_enum_indices[i], compression,
                        start_base_page_id + npages_sum,
        ...
        /* persist compression_base */
        {
            ...
            for (size_t i = 0; i < NFields; ++i)
            {
                ...
                {
                    std::cout << "Field " << i << " has no compression. Skipping persisting compressed base." << std::endl;
                }
                else
                {
                    enum rec_type field_type = tasks[0].field_types[i];
                    ...
                    npages_sum += metadata_set_compression_base_page_id(metadata, table,
                            options.output_fds, options.page_size, tasks[0].field_enum_indices[i],
                            start_base_page_id + npages_sum,
    ...
    for (int i = 0; i < NumRecBufs; ++i) {
        ...
        entry.metrics[i].compression_method = actual_compression_methods[i];
        metadata_set_max_nrows_in_page(metadata, table, tasks[0].field_enum_indices[i], max_nrecs_in_page[i]);
...
uint64_t load_single_file_to_table_column(LoaderOptions &options, SSBTableMetadata &metadata, std::string& filename,
    const std::array<char*, MaxNumBuffers>& buffers,
    ...
    StatEntry &entry)
{
    // Build field_enum_indices from target_fields
    std::array<size_t, NFields> field_enum_indices {};
    for (size_t i = 0; i < NFields; i++) {
        field_enum_indices[i] = static_cast<size_t>(target_fields[i]);
    }
    ...
    for (size_t i = 0; i < NumVarCharFields; ++i) {
        assert(NumVarCharFields == varchar_field_indexes.size());
        std::cout << "VarcharFieldSize[" << i << "] = " << field_sizes[varchar_field_indexes[i]] << std::endl;
    ...
    for (size_t i = 0; i < NFields; ++i) {
        ...
        pagids[i] = start_page_ids[i];
        std::cout << "Allocation from pagids[" << i << "] = " << pagids[i] << std::endl;
        nios[i] = 0;
    }
    ...
    while (std::getline(file, line)) {
        ...
        auto fields = ssb_filter_csv_fields<NFields>(tpch_split_row(line), std::span<const size_t, NFields>(csv_column_map));
        //pagbuf = reinterpret_cast<char*>(buffers[0]);
...
pag_append_retry:
            switch (attr_type) {
                ...
                case rec_type::REC_ATTR_CHAR:
                    ...
                    std::cout << "len[" << f << "]:" << arr_column_sizes[f] << std::endl;
                    // pag_append_rec_unordered_column_char(PAG *pag, size_t schema_len, char *valchar, size_t lenchar, uint64_t rowid, size_t page_size)
                    //rec_set_attr_char(arr_column_sizes[f], recbufs[f], siz_recbuf, row_id, valuechar, part.size());
                    break;
                case rec_type::REC_ATTR_VCHAR:
                    valuechar = const_cast<char*>(part.data());
            ...
            if (slotid == PAG_SLOTID_MASK_ERROR) {
                ...
                if (options.dryrun) {
                    std::cout << "Buffer[0] is full, writing to disk. (pageid=" << pagid << ")\n";
                } else {
                    if (options.verbose) {
    ...
    free(recbuf_base);
    std::cout << "Loaded " << nrecs_loaded << " records from file: " << filename << std::endl;
...
uint64_t bulkload_single_file_column(
    LoaderOptions &options, SSBTableMetadata &metadata, const SSB::common::Table table,
    uint64_t start_page_id,
    const std::array<char*, MaxNumBuffers>& buffers,
    const std::array<size_t, NFields>& field_sizes,
    const std::array<enum rec_type, NFields>& field_types,
    ...
    std::stringstream ss;
    ss << options.input_dirname << "/" << SSB::common::table_name(table) << "/" << SSB::common::table_name(table) << ".tbl";
    std::string path = ss.str();
    ...
    for (size_t i = 0; i < NFields; ++i) {
        auto field_name = SSB::common::metric_field_name(target_fields[i]);
        std::stringstream ss;
    ...
    for (size_t i = 0; i < NFields; ++i) {
        ...
        metadata_set_page_id(metadata, table, field_enum_indices[i], start_page_ids[i], npages[i]);
        /* update stats */
    ...
    for (size_t i = 0; i < NFields; ++i) {
        if (i == 0) {
            std::cout << "Number of pages for " << SSB::common::table_name(table) << " table, "
                << npages << " pages, start_page_ids: " << start_page_ids[i]
                << ", size_total: " << sizes_total[i] << " bytes. "
                << std::endl;
        }
    }
    if (options.enable_dict_encoding) {
        #if 0
        std::cout << "Storage overhead for " << SSB::common::table_name(table) << " table with dictionary encoding: "
    ...
    StatEntry &stat_entry)
{
    ...
    if (table != SSB::common::CUSTOMER && table != SSB::common::LINEORDER
        && table != SSB::common::DDATE
        && table != SSB::common::PART && table != SSB::common::SUPPLIER
    ) {
        std::cerr << "Unknown table type.\n";
        return 0;
    }
    ...
    auto paths = prep_input_files(options, SSB::common::table_name(table));

    // Build field_enum_indices: maps sequential active field index → enum value
    std::array<size_t, NFields> field_enum_indices {};
    for (size_t i = 0; i < NFields; i++) {
        field_enum_indices[i] = static_cast<size_t>(target_fields[i]);
    }

    // SSB::common::fmt.customer_varchar_field_count + 1
    std::cout << options.enable_column << std::endl;
    std::cout << "Number of " << table_name << " files: " << paths.size() << std::endl;
    auto tasks = [&]() {
        if constexpr (std::is_same_v<EnumType, SSB::common::LineOrderField>) {
            return generate_column_tasks<SSB::common::kLineorderSidewaysCount>(
    ...
    std::cout << "Number of tasks files: " << tasks.size() << std::endl;

    for (auto &path : paths) {
        std::cout << "Processing file: " << path << std::endl;
    }

    //nrecs_loaded = load_multiple_files_to_table_column<SSB::common::CustomerField>(
...
#if 0 /* TPCH-specific test/verify functions — disabled for SSB */
void test_rec()
{
    ...
    rec_init(SSB::common::fmt.date_field_sizes, buf, 16384);
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, buf, 16384, 0, 1); // NATIONKEY
    rec_set_attr_char(SSB::common::fmt.date_field_sizes, buf, 16384, 1, country, 9); // NATIONKEY
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, buf, 16384, 2, 1); // NATIONKEY
    rec_set_attr_vchar(SSB::common::fmt.date_field_sizes, buf, 16384, 3, comment, 76); // COMMENT

    rec_print_rec(rp, SSB::common::fmt.date_field_types, stdout);
...
void test_pag()
{
    ...
    // INSERT1
    rec_init(SSB::common::fmt.date_field_sizes, recbuf, 16384);
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, recbuf, 16384, 0, 1); // NATIONKEY
    rec_set_attr_char(SSB::common::fmt.date_field_sizes, recbuf, 16384, 1, country1, 9); // NAME
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, recbuf, 16384, 2, 1); // REGIONKEY
    rec_set_attr_vchar(SSB::common::fmt.date_field_sizes, recbuf, 16384, 3, comment1, 76); // COMMENT

    rec_print_rec(rp, SSB::common::fmt.date_field_types, stdout);
  
    pag_append_rec_unordered(pp, rp, page_size);
    pag_print_pag_data(pp, SSB::common::fmt.date_field_types, page_size, stdout);

    // INSERT2
    rec_init(SSB::common::fmt.date_field_sizes, recbuf, 16384);
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, recbuf, 16384, 0, 2); // NATIONKEY
    rec_set_attr_char(SSB::common::fmt.date_field_sizes, recbuf, 16384, 1, country2, 6); // NAME
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, recbuf, 16384, 2, 1); // REGIONKEY
    rec_set_attr_vchar(SSB::common::fmt.date_field_sizes, recbuf, 16384, 3, comment2, 107); // COMMENT
    ...
    // INSERT3
    rec_init(SSB::common::fmt.date_field_sizes, recbuf, 16384);
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, recbuf, 16384, 0, 3); // NATIONKEY
    rec_set_attr_char(SSB::common::fmt.date_field_sizes, recbuf, 16384, 1, country3, 6); // NAME
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, recbuf, 16384, 2, 1); // REGIONKEY
    rec_set_attr_vchar(SSB::common::fmt.date_field_sizes, recbuf, 16384, 3, comment3, 101); // COMMENT
    ...
    // INSERT4
    rec_init(SSB::common::fmt.date_field_sizes, recbuf, 16384);
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, recbuf, 16384, 0, 4); // NATIONKEY
    rec_set_attr_char(SSB::common::fmt.date_field_sizes, recbuf, 16384, 1, country4, 5); // NAME
    rec_set_attr_int32(SSB::common::fmt.date_field_sizes, recbuf, 16384, 2, 1); // REGIONKEY
    rec_set_attr_vchar(SSB::common::fmt.date_field_sizes, recbuf, 16384, 3, comment4, 99); // COMMENT
...
void test_scan_column_tables_with_sideways_stats(LoaderOptions &options)
{
    ...
    if (options.load_nation) {
        ...
        std::array<uint64_t, SSB::common::N_FIELDS> field_checksums = {};
        for (size_t i = 0; i < SSB::common::N_FIELDS; i++) {
            ...
            uint64_t compression_npages_base = SSB::nbase_to_npages(compression_nbase, options.page_size);
            ...
            for (size_t j = 0; j < nation_npages; j++) {
                ...
                if (compression_method == CompressionMethod::NONE) {
                    ...
                    switch(compression_method) {
                        ...
                        case CompressionMethod::FSST_ROWID:
                        {
                            ...
                            reinterpret_cast<pag_head*>(pag)->nalloc = nrecs;
                            break;
                        }
                        default:
                        {
                ...
                for (size_t l = 0; l < nrecs_in_page; l++) {
                    ...
                    switch (i) {
                        ...
                        case SSB::common::N_NAME:
                        {
                            size_t length = SSB::common::fmt.date_field_sizes[i];
    ...
    if (options.load_region) {
        ...
        std::array<uint64_t, SSB::common::R_FIELDS> field_checksums = {};
        for (size_t i = 0; i < SSB::common::R_FIELDS; i++) {
            // std::cout << metadata.table_customer_start_page_ids[i] << std::endl;
            ...
            for (size_t j = 0; j < region_npages; j++) {
                ...
                for (size_t l = 0; l < nrecs_in_page; l++) {
                    ...
                    switch (i) {
                        ...
                        case SSB::common::R_NAME:
                        {
                            size_t length = SSB::common::fmt.region_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            if (debug_print) {
                                uint64_t rowid = pagcol_get_rowid(pag, l, options.page_size);
                                std::cout << rowid << "|" << std::string_view(col, length) << "|" << std::endl;
    ...
    if (options.load_customer) {
        ...
        std::array<uint64_t, SSB::common::C_FIELDS> field_checksums = {};
        for (size_t i = 0; i < SSB::common::C_FIELDS; i++) {
            ...
            for (size_t j = 0; j < customer_npages; j++) {
                ...
                for (size_t l = 0; l < nrecs_in_page; l++) {
                    switch (i) {
                        ...
                        case SSB::common::C_PHONE:
                        {
                            size_t length = SSB::common::fmt.customer_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            if (debug_print) {
                                uint64_t rowid = pagcol_get_rowid(pag, l, options.page_size);
                                std::cout << rowid << "|" << std::string_view(col, length) << "|" << std::endl;
    ...
    if (options.load_supplier) {
        ...
        std::array<uint64_t, SSB::common::S_FIELDS> field_checksums = {};
        for (size_t i = 0; i < SSB::common::S_FIELDS; i++) {
            ...
            for (size_t j = 0; j < supplier_npages; j++) {
                ...
                for (size_t l = 0; l < nrecs_in_page; l++) {
                    switch (i) {
                        case SSB::common::S_SUPPKEY:
                        {
                            int64_t col = pagcol_fetch_int<int64_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::S_NAME:
                        {
                            size_t length = SSB::common::fmt.supplier_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            auto v = std::string_view(col, length);
                            size_t checksum = std::transform_reduce(
                                v.begin(), v.end(), size_t{0},
                        ...
                        case SSB::common::S_NATIONKEY:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::S_PHONE:
                        {
                            size_t length = SSB::common::fmt.supplier_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            auto v = std::string_view(col, length);
                        ...
                        case SSB::common::S_ACCTBAL:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::S_COMMENT:
                        {
                            char *col = pagcol_fetch_vchar(pag, l, options.page_size);
                            uint16_t length = pagcol_fetch_vchar_len(pag, l, options.page_size);
                            auto v = std::string_view(col, length);
    ...
    if (options.load_part) {
        ...
        std::array<uint64_t, SSB::common::P_FIELDS> field_checksums = {};
        for (size_t i = 0; i < SSB::common::P_FIELDS; i++) {
            ...
            for (size_t j = 0; j < part_npages; j++) {
                ...
                for (size_t l = 0; l < nrecs_in_page; l++) {
                    switch (i) {
                        case SSB::common::P_PARTKEY:
                        {
                            int64_t col = pagcol_fetch_int<int64_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::P_NAME:
                        {
                            char *col = pagcol_fetch_vchar(pag, l, options.page_size);
                            uint16_t length = pagcol_fetch_vchar_len(pag, l, options.page_size);
                            auto v = std::string_view(col, length);
                        ...
                        case SSB::common::P_MFGR:
                        {
                            size_t length = SSB::common::fmt.part_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            auto v = std::string_view(col, length);
                            size_t checksum = std::transform_reduce(
                                v.begin(), v.end(), size_t{0},
                        ...
                        case SSB::common::P_SIZE:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::P_CONTAINER:
                        {
                            size_t length = SSB::common::fmt.part_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            auto v = std::string_view(col, length);
                        ...
                        case SSB::common::P_RETAILPRICE:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::P_COMMENT:
                        {
                            char *col = pagcol_fetch_vchar(pag, l, options.page_size);
                            uint16_t length = pagcol_fetch_vchar_len(pag, l, options.page_size);
                            auto v = std::string_view(col, length);
    ...
    if (options.load_partsupp) {
        ...
        std::array<uint64_t, SSB::common::PS_FIELDS> field_checksums = {};
        for (size_t i = 0; i < SSB::common::PS_FIELDS; i++) {
            ...
            for (size_t j = 0; j < partsupp_npages; j++) {
                ...
                for (size_t l = 0; l < nrecs_in_page; l++) {
                    switch (i) {
                        case SSB::common::PS_PARTKEY:
                        {
                            int64_t col = pagcol_fetch_int<int64_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::PS_SUPPKEY:
                        {
                            int64_t col = pagcol_fetch_int<int64_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::PS_AVAILQTY:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::PS_SUPPLYCOST:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::PS_COMMENT:
                        {
                            char *col = pagcol_fetch_vchar(pag, l, options.page_size);
                            uint16_t length = pagcol_fetch_vchar_len(pag, l, options.page_size);
                            auto v = std::string_view(col, length);
    ...
    if (options.load_orders) {
        ...
        std::array<uint64_t, SSB::common::O_FIELDS> field_checksums = {};
        ...
        for (size_t i = 0; i < SSB::common::O_FIELDS; i++) {
            if (debug_print) std::cout << metadata.table_date_start_page_ids[i] << std::endl;
            size_t orders_start_page_id = metadata.table_date_start_page_ids[i];
            size_t orders_npages = metadata.table_date_npages[i];

            size_t orders_prefix_sum_start_page_id = metadata.table_date_prefix_sum_start_page_ids[i];
            size_t orders_prefix_sum_npages = metadata.table_date_prefix_sum_npages[i];

            size_t orders_stats_start_page_id = metadata.table_date_stats_start_page_ids[i];
            size_t orders_stats_npages = metadata.table_date_stats_npages[i];
            size_t orders_nstats = metadata.table_date_nstats[i];
            ...
            auto& arr_orders_sideways_stats_start_page_id = metadata.table_date_sideways_stats_start_page_ids[i];
            auto& arr_orders_sideways_stats_npages = metadata.table_date_sideways_stats_npages[i];
            auto& arr_orders_sideways_nstats = metadata.table_date_sideways_nstats[i];
            std::array<void *, SSB::common::kOrdersSidewaysCount> ptr_sideways_stats_alloc = {nullptr};
            std::array<std::span<Stats<int32_t>>, SSB::common::kOrdersSidewaysCount> sideways_stats;
            for (size_t l = 0; l < SSB::common::kOrdersSidewaysCount; l++) {
            ...
            uint64_t compressed_page_sizes_start_page_id = metadata.table_date_compressed_page_sizes_start_page_ids[i];
            uint64_t compressed_page_sizes_npages = metadata.table_date_compressed_page_sizes_npages[i];
            uint64_t compression_nbase = metadata.table_date_compression_nbases[i];
            uint64_t compression_npages_base = SSB::nbase_to_npages(compression_nbase, options.page_size);
            uint64_t compressed_base_start_page_ids = metadata.table_date_compression_base_start_page_ids[i];
            CompressionMethod compression_method = static_cast<CompressionMethod>(metadata.table_date_compression_method[i]);
            ...
            for (size_t j = 0; j < orders_npages; j++) {
                ...
                if (i == SSB::common::O_ORDERKEY ||
                    i == SSB::common::O_CUSTKEY ||
                    i == SSB::common::O_TOTALPRICE ||
                    i == SSB::common::O_SHIPPRIORITY) {
                ...
                for (size_t l = 0; l < nrecs_in_page; l++) {
                    ...
                    switch (i) {
                        ...
                        case SSB::common::O_ORDERSTATUS:
                        {
                            size_t length = SSB::common::fmt.orders_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                        ...
                        case SSB::common::O_ORDERPRIORITY:
                        {
                            size_t length = SSB::common::fmt.orders_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            if (enable_stdout) std::cout << (base_row_id + l) << "|" << std::string_view(col, length) << "|" << std::endl;
                        ...
                        case SSB::common::O_SHIPPRIORITY:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            if (enable_stdout) std::cout << (base_row_id + l) << "|" << col << "|" << std::endl;
                            field_checksums[i] += col;
    ...
    if (options.load_lineitem) {
        ...
        /* checksums are initialized with 0 */
        std::array<uint64_t, SSB::common::L_FIELDS> field_checksums = {};
        ...
        for (size_t i = 0; i < SSB::common::L_FIELDS; i++) {
            // std::cout << metadata.table_customer_start_page_ids[i] << std::endl;
            std::cout << metadata.table_lineorder_start_page_ids[i] << std::endl;
            size_t lineitem_start_page_id = metadata.table_lineorder_start_page_ids[i];
            size_t lineitem_npages = metadata.table_lineorder_npages[i];

            size_t lineitem_prefix_sum_start_page_id = metadata.table_lineorder_prefix_sum_start_page_ids[i];
            size_t lineitem_prefix_sum_npages = metadata.table_lineorder_prefix_sum_npages[i];

            size_t lineitem_stats_start_page_id = metadata.table_lineorder_stats_start_page_ids[i];
            size_t lineitem_stats_npages = metadata.table_lineorder_stats_npages[i];
            size_t lineitem_nstats = metadata.table_lineorder_nstats[i];
            ...
            auto& arr_lineitem_sideways_stats_start_page_id = metadata.table_lineorder_sideways_stats_start_page_ids[i];
            auto& arr_lineitem_sideways_stats_npages = metadata.table_lineorder_sideways_stats_npages[i];
            auto& arr_lineitem_sideways_nstats = metadata.table_lineorder_sideways_nstats[i];
            std::array<void *, SSB::common::kLineorderSidewaysCount> ptr_sideways_stats_alloc = {nullptr};
            std::array<std::span<Stats<int32_t>>, SSB::common::kLineorderSidewaysCount> sideways_stats;
            for (size_t l = 0; l < SSB::common::kLineorderSidewaysCount; l++) {
            ...
            uint64_t compressed_page_sizes_start_page_id = metadata.table_lineorder_compressed_page_sizes_start_page_ids[i];
            uint64_t compressed_page_sizes_npages = metadata.table_lineorder_compressed_page_sizes_npages[i];
            uint64_t compression_nbase = metadata.table_lineorder_compression_nbases[i];
            uint64_t compression_npages_base = SSB::nbase_to_npages(compression_nbase, options.page_size);
            uint64_t compressed_base_start_page_ids = metadata.table_lineorder_compression_base_start_page_ids[i];
            CompressionMethod compression_method = static_cast<CompressionMethod>(metadata.table_lineorder_compression_method[i]);
            uint32_t *compressed_page_sizes = nullptr;
            uint64_t *compressed_base_page_ids = nullptr;
            ...
            size_t sum_nrecs = 0;
            size_t total_npages_shipdate = (i == SSB::common::L_SHIPDATE) ? lineitem_npages : 0;
            ...
            for (size_t j = 0; j < lineitem_npages; j++) {
                ...
                // -> 1998-09-02
                if (i == SSB::common::L_RETURNFLAG ||
                    i == SSB::common::L_LINESTATUS ||
                    i == SSB::common::L_QUANTITY ||
                    i == SSB::common::L_EXTENDEDPRICE ||
                    i == SSB::common::L_DISCOUNT) {
                    if (i == SSB::common::L_SHIPDATE) {
                ...
                if (i == SSB::common::L_SHIPDATE ||
                    ...
                    i == SSB::common::L_EXTENDEDPRICE ||
                    i == SSB::common::L_QUANTITY) {
                    if (enable_stats && i == SSB::common::L_SHIPDATE) {
                ...
                if (!(options.lbc_enabled && i == SSB::common::L_COMMENT)) {
                    if (prefix_sum_nrecs[j + 1] - prefix_sum_nrecs[j] != nrecs_in_page) {
                        std::cerr << "Error(" << __FILE__ << ":" << __LINE__ << "): prefix sum for field "
                            << i
                ...
                for (size_t l = 0; l < nrecs_in_page; l++) {
                    ...
                    switch (i) {
                        case SSB::common::L_ORDERKEY:
                        {
                            ...
                            //rec_print_rec(rec, SSB::common::fmt.col_int64_types, stdout);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::L_PARTKEY:
                        {
                            int64_t col = pagcol_fetch_int<int64_t>(pag, l, options.page_size);
                            uint64_t rowid = base_row_id + l;
                            //uint64_t rowid = pagcol_v2_get_rowid<int64_t>(pag, l, options.page_size);
                            ...
                            //uint64_t rowid = pagcol_fetch_vchar_rowid(pag, l, length, options.page_size);
                            //std::cout << rowid << "|" << std::string_view(col, length) << "|" << std::endl;
                            //rec_print_rec(rec, SSB::common::fmt.dict_types, stdout);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::L_SUPPKEY:
                        {
                            int64_t col = pagcol_fetch_int<int64_t>(pag, l, options.page_size);
                            uint64_t rowid = base_row_id + l;
                            if (enable_stdout) std::cout << rowid << "|" << col << "|" << std::endl;
                        ...
                        case SSB::common::L_LINENUMBER:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            uint64_t rowid = base_row_id + l;
                            if (enable_stdout) std::cout << rowid << "|" << col << "|" << std::endl;
                            //rec_print_rec(rec, SSB::common::fmt.col_int32_types, stdout);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::L_QUANTITY:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            uint64_t rowid = base_row_id + l;
                            if (enable_stdout) std::cout << rowid << "|" << col << "|" << std::endl;
                            //rec_print_rec(rec, SSB::common::fmt.col_char_types, stdout);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::L_EXTENDEDPRICE:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            uint64_t rowid = base_row_id + l;
                            if (enable_stdout) std::cout << rowid << "|" << col << "|" << std::endl;
                        ...
                        case SSB::common::L_TAX:
                        {
                            int32_t col = pagcol_fetch_int<int32_t>(pag, l, options.page_size);
                            uint64_t rowid = base_row_id + l;
                            if (enable_stdout) std::cout << rowid << "|" << col << "|" << std::endl;
                            //rec_print_rec(rec, SSB::common::fmt.col_vchar_types, stdout);
                            field_checksums[i] += col;
                            break;
                        }
                        case SSB::common::L_RETURNFLAG:
                        {
                            size_t length = SSB::common::fmt.lineitem_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            uint64_t rowid = base_row_id + l;
                        ...
                        case SSB::common::L_SHIPINSTRUCT:
                        {
                            size_t length = SSB::common::fmt.lineitem_field_sizes[i];
                            char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                            uint64_t rowid = base_row_id + l;
       ...
       // size_t customer_num_varchar_fields = SSB::common::fmt.customer_varchar_field_count;
        // auto &customer_varchar_field_indexes = SSB::common::fmt.customer_varchar_field_indexes;
...
#endif /* TPCH-specific test/verify functions */

/* ===================================================================== */
/*  SSB scan/verify functions for -f option                              */
/*  All SSB active fields are INT32 or INT64 (no VARCHAR/CHAR at         */
/*  storage level due to dictionary encoding).                           */
/* ===================================================================== */

/*
 * Helper: decompress a page in-place into pag buffer.
 * Returns 0 on success, exits on failure.
 */
static void verify_decompress_page(
    CompressionMethod method,
    char *compressed_buffer,
    size_t compressed_page_size,
    PAG *pag,
    char *decomp_workspace,
    size_t page_size)
{
    int ret = 0;
    switch (method) {
        case CompressionMethod::SNAPPY:
            ret = decompress_page_with_snappy(compressed_buffer, compressed_page_size,
                reinterpret_cast<char*>(pag), page_size);
            if (ret != 0) { std::cerr << "decompress_page_with_snappy failed: " << ret << std::endl; exit(EXIT_FAILURE); }
            break;
        case CompressionMethod::DEFLATE:
            ret = decompress_page_with_zlib(compressed_buffer, compressed_page_size,
                reinterpret_cast<char*>(pag), page_size);
            if (ret != 0) { std::cerr << "decompress_page_with_zlib failed: " << ret << std::endl; exit(EXIT_FAILURE); }
            break;
        case CompressionMethod::LZ4:
            ret = decompress_page_with_lz4(compressed_buffer, compressed_page_size,
                reinterpret_cast<char*>(pag), page_size);
            if (ret != 0) { std::cerr << "decompress_page_with_lz4 failed: " << ret << std::endl; exit(EXIT_FAILURE); }
            break;
        case CompressionMethod::LZ4PAR:
            ret = decompress_page_with_lz4par(compressed_buffer, compressed_page_size,
                reinterpret_cast<char*>(pag), page_size);
            if (ret != 0) { std::cerr << "decompress_page_with_lz4par failed: " << ret << std::endl; exit(EXIT_FAILURE); }
            break;
        case CompressionMethod::PFOR:
            ret = decompress_page_with_pfor(compressed_buffer, compressed_page_size,
                reinterpret_cast<char*>(pag), decomp_workspace, page_size);
            if (ret != 0) { std::cerr << "decompress_page_with_pfor failed: " << ret << std::endl; exit(EXIT_FAILURE); }
            break;
        case CompressionMethod::PFOR64:
            ret = decompress_page_with_pfor64(compressed_buffer, compressed_page_size,
                reinterpret_cast<char*>(pag), decomp_workspace, page_size);
            if (ret != 0) { std::cerr << "decompress_page_with_pfor64 failed: " << ret << std::endl; exit(EXIT_FAILURE); }
            break;
        case CompressionMethod::FSST_ROWID:
        ...
        default:
            std::cerr << "Unknown compression method: " << static_cast<int>(method) << std::endl;
            exit(EXIT_FAILURE);
    }
}

/*
 * Verify a single table's column store pages.
 *
 * Parameters:
 *   table_name        - human-readable name for output
 *   nfields           - number of active fields
 *   field_types       - REC_ATTR_INT32 or REC_ATTR_INT64 per active field
 *   field_sizes       - sizeof(int32_t) or sizeof(int64_t) per active field
 *   start_page_ids    - metadata: per-field start page ID
 *   npages_arr        - metadata: per-field number of pages
 *   compression_method_arr - metadata: per-field compression method
 *   compressed_page_sizes_start_page_ids - metadata
 *   compressed_page_sizes_npages         - metadata
 *   compression_nbases                   - metadata
 *   compression_base_start_page_ids      - metadata
 *   prefix_sum_start_page_ids            - metadata
 *   prefix_sum_npages                    - metadata
 *   options           - loader options (page_size, output_fds, etc.)
 */
static void verify_scan_table(
    const char *table_name,
    size_t nfields,
    const enum rec_type *field_types,
    const size_t *field_sizes,
    const uint64_t *start_page_ids,
    const uint64_t *npages_arr,
    const uint16_t *compression_method_arr,
    const uint64_t *compressed_page_sizes_start_page_ids,
    const uint64_t *compressed_page_sizes_npages,
    const uint64_t *compression_nbases,
    const uint64_t *compression_base_start_page_ids,
    const uint64_t *prefix_sum_start_page_ids,
    const uint64_t *prefix_sum_npages,
    LoaderOptions &options)
{
    constexpr bool debug_print = false;
    ...
    std::cout << "\n=== Verifying " << table_name << " (" << nfields << " active fields) ===" << std::endl;

    std::vector<uint64_t> field_checksums(nfields, 0);

    for (size_t i = 0; i < nfields; i++) {
        size_t tbl_start_page_id = start_page_ids[i];
        size_t tbl_npages = npages_arr[i];

        CompressionMethod compression_method = static_cast<CompressionMethod>(compression_method_arr[i]);

        if (debug_print) {
            std::cout << "  field[" << i << "]: start_page_id=" << tbl_start_page_id
                << " npages=" << tbl_npages
                << " compression=" << compression_method_name(compression_method)
                << std::endl;
        }

        /* Read prefix sums to verify row counts */
        void *ptr_prefix_sum_alloc = nullptr;
        uint64_t *prefix_sum_nrecs = nullptr;
        if (prefix_sum_npages[i] > 0) {
            if (posix_memalign((void**)&ptr_prefix_sum_alloc, 512, prefix_sum_npages[i] * options.page_size) != 0) {
                std::cerr << "posix_memalign failed" << std::endl;
                exit(EXIT_FAILURE);
            }
            prefix_sum_nrecs = reinterpret_cast<uint64_t*>(ptr_prefix_sum_alloc);
            for (size_t j = 0; j < prefix_sum_npages[i]; j++) {
                page_pread_host(options.output_fds,
                    reinterpret_cast<char*>(prefix_sum_nrecs) + j * options.page_size,
                    prefix_sum_start_page_ids[i] + j, options.page_size);
            }
        }

        /* Compression metadata */
        uint32_t *compressed_page_sizes_arr_data = nullptr;
        uint64_t *compressed_base_page_ids = nullptr;
        std::vector<size_t> c_offsets;
        char *compressed_buffer = nullptr;
        char *decomp_workspace = nullptr;
        ...
        if (compression_method != CompressionMethod::NONE) {
            uint64_t comp_npages = compressed_page_sizes_npages[i];
            uint64_t comp_nbase = compression_nbases[i];
            uint64_t comp_npages_base = SSB::nbase_to_npages(comp_nbase, options.page_size);

            if (posix_memalign((void**)&ptr_compressed_page_sizes, 512, comp_npages * options.page_size) != 0) {
                std::cerr << "posix_memalign failed" << std::endl;
                exit(EXIT_FAILURE);
            }
            if (posix_memalign((void**)&ptr_compression_offset_base_page_ids, 512, comp_npages_base * options.page_size) != 0) {
                std::cerr << "posix_memalign failed" << std::endl;
                exit(EXIT_FAILURE);
            }
            for (size_t j = 0; j < comp_npages; j++) {
                page_pread_host(options.output_fds,
                    reinterpret_cast<char*>(ptr_compressed_page_sizes) + j * options.page_size,
                    compressed_page_sizes_start_page_ids[i] + j, options.page_size);
            }
            for (size_t j = 0; j < comp_npages_base; j++) {
                page_pread_host(options.output_fds,
                    reinterpret_cast<char*>(ptr_compression_offset_base_page_ids) + j * options.page_size,
                    compression_base_start_page_ids[i] + j, options.page_size);
            }
            if (posix_memalign((void**)&ptr_compressed_buffer, 512, options.page_size) != 0) {
                std::cerr << "posix_memalign failed" << std::endl;
                exit(EXIT_FAILURE);
            ...
            compressed_page_sizes_arr_data = reinterpret_cast<uint32_t*>(ptr_compressed_page_sizes);
            compressed_buffer = reinterpret_cast<char*>(ptr_compressed_buffer);
            compressed_base_page_ids = reinterpret_cast<uint64_t*>(ptr_compression_offset_base_page_ids);

            calculate_compressed_offsets(
                reinterpret_cast<size_t *>(compressed_base_page_ids),
                compressed_page_sizes_arr_data,
                comp_nbase,
                tbl_npages,
                options.page_size,
                tbl_start_page_id,
        ...
        size_t sum_nrecs = 0;
        for (size_t j = 0; j < tbl_npages; j++) {
            const uint64_t page_id = tbl_start_page_id + j;

            if (compression_method == CompressionMethod::NONE) {
                page_pread_host(options.output_fds, (void*)pag, page_id, options.page_size);
            } else {
                size_t compressed_page_size = compressed_page_sizes_arr_data[j];
                size_t read_size = roundup4096(compressed_page_size);
                uint64_t offset = calc_compressed_page_offset(page_id, c_offsets.data(), tbl_start_page_id);
                read_compressed_page_host(options.output_fds, compressed_buffer, page_id, read_size, options.page_size, offset);
                verify_decompress_page(compression_method, compressed_buffer, compressed_page_size,
                    pag, decomp_workspace, options.page_size);
            }

            size_t nrecs_in_page = pag_get_nalloc(pag);

            /* Verify prefix sum consistency */
            if (prefix_sum_nrecs != nullptr) {
                if (prefix_sum_nrecs[j + 1] - prefix_sum_nrecs[j] != nrecs_in_page) {
                    std::cerr << "Error: " << table_name << " field[" << i << "] page " << j
                        << " prefix sum diff=" << (prefix_sum_nrecs[j + 1] - prefix_sum_nrecs[j])
                        << " but nrecs_in_page=" << nrecs_in_page << std::endl;
                    exit(EXIT_FAILURE);
                }
            }

            /* Compute field checksum */
            bool is_char_as_int = (field_types[i] == REC_ATTR_CHAR && field_sizes[i] < 3
                                   && compression_method == CompressionMethod::PFOR);
            for (size_t l = 0; l < nrecs_in_page; l++) {
                if (field_types[i] == REC_ATTR_CHAR && !is_char_as_int) {
                    size_t length = field_sizes[i];
                    char *col = pagcol_fetch_char(pag, l, length, options.page_size);
                    auto v = std::string_view(col, length);
                    size_t checksum = std::transform_reduce(
                        v.begin(), v.end(), size_t{0},
                        ...
                        [](char c) { return static_cast<size_t>(c); });
                    field_checksums[i] += checksum;
                } else if (field_types[i] == REC_ATTR_INT64) {
                    int64_t col = pagcol_fetch_int<int64_t>(pag, l, options.page_size);
                    field_checksums[i] += col;
                } else {
                    /* INT32 */
        ...
        std::cout << "  field[" << i << "]: nrecs=" << sum_nrecs
            << " compression=" << compression_method_name(compression_method)
            << " checksum=" << field_checksums[i] << std::endl;
    ...
    std::cout << table_name << " checksums:" << std::endl;
    for (size_t i = 0; i < nfields; i++) {
        std::cout << "\tfield[" << i << "] = " << field_checksums[i] << std::endl;
    }
...
void test_scan_column_tables(LoaderOptions &options)
{
    ...
    page_pread_host(options.output_fds, ptr, 0, options.page_size);
    SSBTableMetadata *metadatap = reinterpret_cast<SSBTableMetadata*>(ptr);
    SSBTableMetadata &metadata = *metadatap;

    std::cout << "=== SSB Verify: reading back column data ===" << std::endl;

    /* DATE (3 active fields, all INT32) */
    if (options.load_ddate) {
        verify_scan_table("DATE",
            SSB::common::kDateFieldCount,
            SSB::common::fmt.date_field_types.data(),
            SSB::common::fmt.date_field_sizes.data(),
            metadata.table_date_start_page_ids,
            metadata.table_date_npages,
            metadata.table_date_compression_method,
            metadata.table_date_compressed_page_sizes_start_page_ids,
            metadata.table_date_compressed_page_sizes_npages,
            metadata.table_date_compression_nbases,
            metadata.table_date_compression_base_start_page_ids,
            metadata.table_date_prefix_sum_start_page_ids,
            metadata.table_date_prefix_sum_npages,
            options);
    }

    /* CUSTOMER (4 active fields, all INT32) */
    if (options.load_customer) {
        verify_scan_table("CUSTOMER",
            SSB::common::kCustomerFieldCount,
            SSB::common::fmt.customer_field_types.data(),
            SSB::common::fmt.customer_field_sizes.data(),
            metadata.table_customer_start_page_ids,
            metadata.table_customer_npages,
            metadata.table_customer_compression_method,
            metadata.table_customer_compressed_page_sizes_start_page_ids,
            metadata.table_customer_compressed_page_sizes_npages,
            metadata.table_customer_compression_nbases,
            metadata.table_customer_compression_base_start_page_ids,
            metadata.table_customer_prefix_sum_start_page_ids,
            metadata.table_customer_prefix_sum_npages,
            options);
    }

    /* SUPPLIER (4 active fields, all INT32) */
    if (options.load_supplier) {
        verify_scan_table("SUPPLIER",
            SSB::common::kSupplierFieldCount,
            SSB::common::fmt.supplier_field_types.data(),
            SSB::common::fmt.supplier_field_sizes.data(),
            metadata.table_supplier_start_page_ids,
            metadata.table_supplier_npages,
            metadata.table_supplier_compression_method,
            metadata.table_supplier_compressed_page_sizes_start_page_ids,
            metadata.table_supplier_compressed_page_sizes_npages,
            metadata.table_supplier_compression_nbases,
            metadata.table_supplier_compression_base_start_page_ids,
            metadata.table_supplier_prefix_sum_start_page_ids,
            metadata.table_supplier_prefix_sum_npages,
            options);
    }

    /* PART (4 active fields, all INT32) */
    if (options.load_part) {
        verify_scan_table("PART",
            SSB::common::kPartFieldCount,
            SSB::common::fmt.part_field_types.data(),
            SSB::common::fmt.part_field_sizes.data(),
            metadata.table_part_start_page_ids,
            metadata.table_part_npages,
            metadata.table_part_compression_method,
            metadata.table_part_compressed_page_sizes_start_page_ids,
            metadata.table_part_compressed_page_sizes_npages,
            metadata.table_part_compression_nbases,
            metadata.table_part_compression_base_start_page_ids,
            metadata.table_part_prefix_sum_start_page_ids,
            metadata.table_part_prefix_sum_npages,
            options);
    }

    /* LINEORDER (14 active fields, LO_ORDERKEY=INT64, rest=INT32) */
    if (options.load_lineorder) {
        verify_scan_table("LINEORDER",
            SSB::common::kLineOrderFieldCount,
            SSB::common::fmt.lineorder_field_types.data(),
            SSB::common::fmt.lineorder_field_sizes.data(),
            metadata.table_lineorder_start_page_ids,
            metadata.table_lineorder_npages,
            metadata.table_lineorder_compression_method,
            metadata.table_lineorder_compressed_page_sizes_start_page_ids,
            metadata.table_lineorder_compressed_page_sizes_npages,
            metadata.table_lineorder_compression_nbases,
            metadata.table_lineorder_compression_base_start_page_ids,
            metadata.table_lineorder_prefix_sum_start_page_ids,
            metadata.table_lineorder_prefix_sum_npages,
            options);
    }

    std::cout << "\n=== SSB Verify complete ===" << std::endl;
