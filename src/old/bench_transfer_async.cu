...
struct TransferAsyncThreadArgs
{
    size_t thrid;
    cudaStream_t stream;
    void *buf_host;
...
BenchmarkResult bench_transfer_async(const BenchmarkOptions &options)
{
    ...
    std::vector<TransferAsyncThreadArgs> thread_args_vec;
    ...
    for (size_t i = 0; i < options.nthreads; i++)
    {
        ...
        thread_args_vec.push_back(TransferAsyncThreadArgs{
            .thrid = i,
            .stream = stream,
            .buf_host = buf_host,
            .buf_dev = buf_dev,
    ...
    if (options.benchmark_type == BenchmarkType::HOST_TO_DEVICE)
    {
        for (size_t i = 0; i < options.nthreads; i++)
        {
            TransferAsyncThreadArgs &args = thread_args_vec[i];
            threads.emplace_back(
                [&args]()
                {
                    ...
                    for (size_t j = 0; j < args.nios; j++)
                    {
                        ...
                        if ((j + 1) % args.io_depth == 0 || j + 1 == args.nios)
                        {
