...
struct DeviceBatchCookie
{
    void *buf_dev;
    size_t idx;
};

struct DeviceBatchThreadArgs
{
    size_t thrid;
    ...
    std::vector<DeviceBatchCookie> batch_cookie_vec;
    uint64_t period_sec;
    size_t stats_nio;
};

BenchmarkResult bench_device_batch(BenchmarkOptions &options)
{
    std::vector<int> fds;
    std::vector<CUfileHandle_t> cufile_handles;
    ...
    std::vector<DeviceBatchThreadArgs> thread_args_vec;
    ...
    for (size_t i = 0; i < options.nthreads; i++)
    {
        ...
        std::vector<DeviceBatchCookie> batch_cookie_vec(nios_per_thread);
        ...
        thread_args_vec.push_back(DeviceBatchThreadArgs{
            .thrid = i,
    ...
    for (size_t i = 0; i < options.nthreads; i++)
    {
        DeviceBatchThreadArgs &args = thread_args_vec[i];
                ...
                while (done < nios)
                {
                    ...
                    if (ongoing < options.io_depth && done + ongoing < nios)
                    {
                        ...
                        for (; j < std::min(options.io_depth - ongoing, nios - (done + ongoing)); j++)
                        {
                            ...
                            DeviceBatchCookie *cookie = &args.batch_cookie_vec[done + ongoing + j];
