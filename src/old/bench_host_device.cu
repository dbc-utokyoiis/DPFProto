...
BenchmarkResult bench_host_device(BenchmarkOptions &options)
{
    std::vector<int> fds;
    ...
    std::vector<HostDeviceThreadArgs> thread_args_vec;
    ...
    for (size_t i = 0; i < options.nthreads; i++)
    {
        ...
        thread_args_vec.push_back(HostDeviceThreadArgs{
            .thrid = i,
            .fds = fds,
            .buf_host = buf_host,
            .buf_dev = buf_dev,
        ...
        options.benchmark_type == BenchmarkType::RANDOM_READ)
    {
        for (size_t i = 0; i < options.nthreads; i++)
        {
            HostDeviceThreadArgs &args = thread_args_vec[i];
