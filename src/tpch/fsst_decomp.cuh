...
// ============================================================
// Fused FSST decomp + KMP scan + Q13 custkey write
//
// Same as decompress_scan_string_with_fsst but additionally
// writes custkey for qualifying (NOT LIKE) records:
//   d_o_aggr_custkey[rec_base+r] = d_o_custkey_flat[rec_base+r]
// d_o_aggr_custkey must be pre-filled with UINT64_MAX (0xFF).
// ============================================================

__global__ void decompress_scan_string_with_fsst_q13(
    const char*                    d_pages,
    const FsstCompBlockKernelMeta* d_cb_meta,
    uint32_t                       total_comp_blocks,
    const char*                    d_patterns,
    ...
    uint64_t*                      d_count)
{
    ...
    for (uint32_t r = tid; r < nrecs; r += blockDim.x) {
        ...
        if (!done) {
            my_qualifying++;
            // Q13: write custkey for qualifying (NOT LIKE) records
            d_o_aggr_custkey[rec_base + r] = d_o_custkey_flat[rec_base + r];
        }
    }
